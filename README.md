...
- **LoRA**: "LoRA 관리"에서 스타일·아티스트·사운드 LoRA를 "Preset" 탭(종류·설명·추천 강도·트리거 단어 포함)나 허깅페이스(태그·언어·라이선스·샘플로 분류)에서 받고, 이름·메모로 관리합니다. 곡 만들기의 "고급 설정"에서 고르면 작곡 강도와 사운드 강도를 따로 조절할 수 있고, 그 곡은 yue-server 엔진으로 만들어집니다(LoRA를 쓰지 않는 곡은 기존 그대로). 필요한 파일은 `docs/models.md`의 "LoRA 엔진"을 보세요.
...
- **⑨ Audio Tools 엔진 (audio.cpp, 선택)**: "오디오 도구" 페이지의 TTS(Qwen3-TTS·OmniVoice·Fish Audio·Supertonic 3·MagpieTTS·Chatterbox)와 음성 인식(Qwen3-ASR·Nemotron 3.5·VibeVoice-ASR)은 audio.cpp로 실행합니다. 모델은 앱에서 선택해 내려받고(미설치 시 "모델 받기"), 음성 조절(피치·속도·음량·노이즈)은 ffmpeg로 처리합니다. (이전의 AudioAuK 연동은 2026-09-24 제거되었습니다.)
...
- **TTS 생성**: T2S(음색 설명) / Ref-T2S(참조 목소리 복제) / 프리셋 목소리 3가지. 모델·크기·정밀도를 화면에서 고르고 미설치 조합은 "모델 받기"로 내려받습니다(`models/audio-cpp/audio.cpp-gguf/`).
  - T2S: Qwen3-TTS VoiceDesign, (Chatterbox는 Qwen3가 만든 참조 음성 사용), Typecast
  - Ref-T2S: Qwen3-TTS Base, OmniVoice, Fish Audio S2 Pro, Chatterbox, Typecast(유료 플랜)
