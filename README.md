...
- **LoRA**: "LoRA 관리"에서 스타일·아티스트·사운드 LoRA를 카탈로그(종류·설명·추천 강도·트리거 단어 포함)나 허깅페이스(태그·언어·라이선스·샘플로 분류)에서 받고, 이름·메모로 관리합니다. 곡 만들기의 "고급 설정"에서 고르면 작곡 강도와 사운드 강도를 따로 조절할 수 있고, 그 곡은 yue-server 엔진으로 만들어집니다(LoRA를 쓰지 않는 곡은 기존 그대로). 필요한 파일은 `docs/models.md`의 "LoRA 엔진"을 보세요.
