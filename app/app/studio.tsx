...
import { AudioLines, ArrowDownToLine, ArrowRight, Check, ChevronDown, ChevronLeft, ChevronRight, CircleHelp, Cpu, Dices, Download, FastForward, FileText, Folder, Guitar, Headphones, Heart, Home, Image as ImageIcon, LayoutGrid, ListMusic, ListPlus, LoaderCircle, Menu, Mic, MoreVertical, Music2, Pause, Pencil, Play, Plus, RefreshCw, Rewind, RotateCcw, Save, Search, Settings2, ShieldCheck, SkipBack, SkipForward, SlidersHorizontal, Sparkles, Square, Trash2, Upload, Volume2, WandSparkles, X } from 'lucide-react';
...
export default function Studio() {
  ...
  async function saveDraft(generate = false) {
    if ((!draft.instrumental && !draft.lyrics.trim()) || !draft.style.trim()) { notify('가사와 음악 스타일을 입력해 주세요. 예시를 불러와 시작해도 좋아요.', true); return; }
  ...
  async function runPlan() {
    if ((!draft.instrumental && !draft.lyrics.trim()) || !draft.style.trim()) { notify('가사와 음악 스타일을 입력해 주세요.', true); return; }
  ...
  async function saveAbcToLibrary() {
    if (!draft.abc.trim()) { notify('저장할 악보가 없습니다.', true); return; }
    try {
      if (currentAbcNoteId) {
        const note = await api<AbcNote>(`/abc-notes/${currentAbcNoteId}`, 'PATCH', { abc: draft.abc });
        setAbcNotes(previous => previous.map(item => item.id === note.id ? note : item));
        notify(`"${note.title}" 악보를 갱신했습니다.`);
      } else {
        const note = await api<AbcNote>('/abc-notes', 'POST', { title: draft.title.trim() || '제목 없는 악보', abc: draft.abc });
        setAbcNotes(previous => [...previous, note]);
        setCurrentAbcNoteId(note.id);
        notify('악보를 라이브러리에 저장했습니다. 다시 저장하면 이 악보를 덮어씁니다.');
      }
    } catch (error) { notify((error as Error).message, true); }
  }
  ...
  return <div className="studio-shell">
    ...
    <div className="form-section"><div className="field-heading"><label htmlFor="lyrics"><FileText size={16}/>가사</label><button className="text-action" onClick={() => void assist('lyrics')} disabled={!!busy || draft.instrumental}><WandSparkles size={13}/>작사 도우미</button></div><div className="lyrics-box"><Textarea id="lyrics" value={draft.lyrics} onChange={event => update({ lyrics: event.target.value })} disabled={draft.instrumental} placeholder={'[Verse]\n이곳에 나만의 이야기를 적어 주세요.\n직접 쓴 가사를 붙여 넣어도 좋아요.\n\n[Chorus]\n마음에 남을 후렴을 들려주세요.'} maxLength={12000}/><div className="textarea-footer"><span>{draft.lyrics.length.toLocaleString()} / 12,000</span></div></div>{draft.instrumental && <p className="field-hint">악기만 모드에서는 가사가 생성에 사용되지 않습니다. 작성해 둔 가사는 그대로 보존되며, "보컬+악기"로 돌아가면 다시 사용할 수 있어요.</p>}</div>
