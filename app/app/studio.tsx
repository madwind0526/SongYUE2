...
async function loadCompressorPresets(): Promise<Record<string, CompressorValues>> {
  try {
    ...
    return Object.fromEntries(list.map(preset => [preset.name, preset.values]));
  } catch { return {}; }
}
...
function finishBuffer(input: AudioBuffer, p: PostProcessParams): AudioBuffer {
  ...
  if (useReverse) for (const channel of data) channel.reverse();
  let outRate = rate;
  if (useSpeed) {
  ...
  if (useFade && p.fadeInSec > 0) { const n = Math.min(length, Math.round(p.fadeInSec * rate)); for (const channel of data) for (let i = 0; i < n; i++) channel[i] *= i / n; }
  if (useFade && p.fadeOutSec > 0) { const n = Math.min(length, Math.round(p.fadeOutSec * rate)); for (const channel of data) for (let i = 0; i < n; i++) channel[length - 1 - i] *= i / n; }
  const output = new AudioBuffer({ length: Math.max(1, length), numberOfChannels: channels, sampleRate: outRate });
...
function EqBar({ label, value, onChange, off }: { label: string; value: number; onChange: (next: number) => void; off?: boolean }) {
  ...
  return <div className={off ? "eq-bar-col pp-off" : "eq-bar-col"}>
...
function Knob({ label, value, min, max, onChange, variant, step = 5, off }: { label: string; value: number; min: number; max: number; onChange: (next: number) => void; variant?: 'fx' | 'reverb' | 'comp'; step?: number; off?: boolean }) {
  ...
  return <div className={`${variant ? `knob knob-${variant}` : 'knob'}${off ? ' pp-off' : ''}`}>
...
function PostProcessDialog({ project, onClose, notify, visualizerEnabled, visualizerRingCount, visualizerHue, visualizerLineWidth, visualizerTrail, visualizerSpiral, visualizerRingMode, visualizerTimeStep, visualizerTimeSkew, visualizerRingStep, visualizerAmplitude, sourceOverride, onSaveOverride, titleOverride }: { project: Project; onClose: () => void; notify: (text: string, error?: boolean) => void; visualizerEnabled: boolean; visualizerRingCount: number; visualizerHue: number; visualizerLineWidth: number; visualizerTrail: number; visualizerSpiral: number; visualizerRingMode: 'radial' | 'time'; visualizerTimeStep: number; visualizerTimeSkew: number; visualizerRingStep: number; visualizerAmplitude: number; sourceOverride?: { buffer: AudioBuffer; params: PostProcessParams }; onSaveOverride?: (buffer: AudioBuffer, params: PostProcessParams) => void; titleOverride?: string }) {
  ...
  async function saveCompPreset() {
    ...
    if (!name) return;
    if (name in COMPRESSOR_PRESETS) { notify('기본 프리셋과 같은 이름은 사용할 수 없습니다.', true); return; }
    try {
      const saved = await api<{ name: string; values: CompressorValues }>('/compressor-presets', 'POST', { name, values: compValues() });
      setCustomCompPresets(previous => ({ ...previous, [saved.name]: saved.values }));
      setCompPreset(saved.name);
      notify(`"${saved.name}" 컴프레서 프리셋으로 저장했습니다.`);
    } catch (error) { notify((error as Error).message, true); }
  }
  async function deleteCompPreset(name: string) {
    try {
      ...
      {loading ? <p className="field-hint">오디오를 불러오는 중...</p> : <>
        <div className="pp-layout">
          <div className="pp-eq-panel">
            ...
            <div className="pp-eq-bars">{PP_EQ_BANDS.map((_, index) => <EqBar key={PP_EQ_LABELS[index]} label={PP_EQ_LABELS[index]} value={params.eq[index]} off={!params.eqEnabled} onChange={value => updateEq(index, value)}/>)}</div>
          ...
          <div className="pp-fx-panel">
            ...
            <div className="pp-knob-grid">
              ...
              <Knob label="서라운드 사운드" off={!params.fxEnabled} value={params.surround} min={-100} max={100} onChange={value => updateParam('surround', value)} variant="fx"/>
              <Knob label="다이내믹 부스트" off={!params.fxEnabled} value={params.dynamicBoost} min={0} max={100} onChange={value => updateParam('dynamicBoost', value)} variant="fx"/>
              <Knob label="베이스 부스트" off={!params.fxEnabled} value={params.bassBoost} min={-100} max={100} onChange={value => updateParam('bassBoost', value)} variant="fx"/>
              <Knob label="리버브 양" off={!params.reverbEchoEnabled} value={params.reverbAmount} min={0} max={100} onChange={value => updateParam('reverbAmount', value)} variant="reverb"/>
        ...
        <div className="pp-fx-panel pp-extra-panel">
          <div className="pp-fx-header">
            <div className="pp-extra-toggles">
              <div className={params.volumeOn ? 'pp-toggle-btn active' : 'pp-toggle-btn'}><button type="button" className="pp-toggle-power" onClick={() => updateParam('volumeOn', !params.volumeOn)}><Power size={12}/>음량</button></div>
...
const STEM_MODE_CONFIG = {
  channel: { stems: ['left', 'right'] as const, menuSub: '', description: '왼쪽·오른쪽 채널 2갈래(AI 분리 모델 없이 순수 채널 분리)', dialogTitle: '채널 분리', failureHint: '채널 분리에 실패했습니다. ffmpeg가 설치되어 있는지, 원본이 스테레오인지 확인해 주세요.' },
  full: { stems: ['vocals', 'drums', 'bass', 'other'] as const, menuSub: '4CH', description: '보컬·드럼·베이스·그 외 악기 4갈래', dialogTitle: 'STEM 분리', failureHint: 'STEM 분리에 실패했습니다. audio.cpp가 해당 분리 모델을 포함해 빌드되어 있는지, 모델 파일이 있는지 확인해 주세요.' },
  full6: { stems: ['vocals', 'drums', 'bass', 'guitar', 'piano', 'other'] as const, menuSub: '6CH', description: '보컬·드럼·베이스·기타(guitar)·피아노·그 외 악기 6갈래', dialogTitle: 'STEM 분리', failureHint: 'STEM 분리에 실패했습니다. audio.cpp가 해당 분리 모델을 포함해 빌드되어 있는지, 모델 파일이 있는지 확인해 주세요.' },
  vocal: { stems: ['vocals', 'instrumental'] as const, menuSub: '2CH', description: '보컬·악기 2갈래(보컬 누출이 적은 분리 모델)', dialogTitle: 'STEM 분리', failureHint: 'STEM 분리에 실패했습니다. audio.cpp가 해당 분리 모델을 포함해 빌드되어 있는지, 모델 파일이 있는지 확인해 주세요.' },
};
type StemMode = keyof typeof STEM_MODE_CONFIG;
type StemName = typeof STEM_MODE_CONFIG[StemMode]['stems'][number];
const STEM_LABELS: Record<StemName, string> = { vocals: '보컬', drums: '드럼', bass: '베이스', other: '그 외 악기', guitar: '기타 (guitar)', piano: '피아노', instrumental: '악기', left: '왼쪽 채널', right: '오른쪽 채널' };
...
function StemDialog({ project, mode, onClose, notify, visualizerEnabled, visualizerRingCount, visualizerHue, visualizerLineWidth, visualizerTrail, visualizerSpiral, visualizerRingMode, visualizerTimeStep, visualizerTimeSkew, visualizerRingStep, visualizerAmplitude }: { project: Project; mode: StemMode; onClose: () => void; notify: (text: string, error?: boolean) => void; visualizerEnabled: boolean; visualizerRingCount: number; visualizerHue: number; visualizerLineWidth: number; visualizerTrail: number; visualizerSpiral: number; visualizerRingMode: 'radial' | 'time'; visualizerTimeStep: number; visualizerTimeSkew: number; visualizerRingStep: number; visualizerAmplitude: number }) {
  ...
  const multiStem = mode === 'full' || mode === 'full6'; // the 4- and 6-way modes share the per-stem checkbox behaviour
  const [loading, setLoading] = useState(true);
  const [errorText, setErrorText] = useState('');
  ...
  function handleCombinedRowClick(key: string) {
    if (multiStem) setAudibleStems(new Set(stemOrder));
    handleKeyClick(key);
  }
  ...
  return <>
    {!editingStem && <Dialog open onOpenChange={open => { if (!open) { stopPlayback(); onClose(); } }}>
      <DialogContent className="studio-dialog stem-dialog">
        <DialogTitle>{STEM_MODE_CONFIG[mode].dialogTitle}{STEM_MODE_CONFIG[mode].menuSub ? ` (${STEM_MODE_CONFIG[mode].menuSub})` : ''}</DialogTitle>
        ...
        <div className="dialog-actions pp-dialog-actions">
          <div className="pp-transport">
            <Button variant="ghost" size="icon" aria-label="5초 뒤로" onClick={() => seekBy(-5)} disabled={loading}><Rewind size={15}/></Button>
            <Button variant="ghost" size="icon" aria-label={isPlaying ? '일시정지' : '재생'} onClick={() => handleKeyClick(activeKey || (multiStem ? 'combined-preview' : 'combined-original'))} disabled={loading}>{isPlaying ? <Pause size={15}/> : <Play size={15}/>}</Button>
            <Button variant="ghost" size="icon" aria-label="5초 앞으로" onClick={() => seekBy(5)} disabled={loading}><FastForward size={15}/></Button>
            <button type="button" className="speed-btn" aria-label="재생 속도" onClick={cycleSpeed} disabled={loading}>{playbackRate}x</button>
            <Volume2 size={14} className="pp-volume-icon"/>
            <input className="abc-player-volume" type="range" aria-label="미리듣기 볼륨" min={0} max={2} step={0.1} value={previewVolume} onChange={event => applyPreviewVolume(Number(event.target.value))} disabled={loading}/>
          ...
          <div className="pp-dialog-actions-right">
            ...
            <Button onClick={() => void handleMerge()} disabled={loading || !!errorText || merging || (multiStem && audibleStems.size === 0)}>{merging ? <LoaderCircle className="spin"/> : <Combine size={15}/>}합치기</Button>
...
// "가사 싱크": 곡의 가사를 보컬 소리에 맞춰 줄 단위 시간(LRC)으로 만든다. 서버가 보컬을 분리하고 음성 인식/단어 정렬로
// 들린 단어의 시간을 구한 뒤 곡의 가사 줄에 매칭한다. 결과는 곡 옆에 저장되어 플레이어가 재생 중 가사를 보여 준다.
type LyricSyncLine = { text: string; start: number; end: number; confidence: number; estimated: boolean };
type LyricSync = { language: string; coverage: number; stale: boolean; lines: LyricSyncLine[] };
function currentLyricIndex(lines: LyricSyncLine[], time: number): number {
  let index = -1;
  for (let i = 0; i < lines.length; i += 1) { if (lines[i].start <= time + 0.05) index = i; else break; }
  return index;
}
function LyricsSyncDialog({ project, onClose, notify }: { project: Project; onClose: () => void; notify: (text: string, error?: boolean) => void }) {
  const [sync, setSync] = useState<LyricSync | null>(null);
  const [loading, setLoading] = useState(true);
  const [running, setRunning] = useState(false);
  const [errorText, setErrorText] = useState('');
  ...
  const hasLyrics = /[^\s\[\]]/.test((project.lyrics || '').replace(/^\[[^\]]*\]$/gm, ''));
  const current = sync ? currentLyricIndex(sync.lines, time) : -1;
  const languageName: Record<string, string> = { ko: '한국어', ja: '일본어', zh: '중국어', en: '영어' };

  useEffect(() => {
    let cancelled = false;
    api<LyricSync>(`/projects/${project.id}/lyrics-sync`).then(result => { if (!cancelled) setSync(result); }).catch(() => {}).finally(() => { if (!cancelled) setLoading(false); });
    return () => { cancelled = true; };
  }, [project.id]);
  ...
  async function make() {
    ...
    try { setSync(await api<LyricSync>(`/projects/${project.id}/lyrics-sync`, 'POST', {})); }
    catch (error) { setErrorText((error as Error).message); }
    finally { setRunning(false); }
  }
  async function saveLrc() {
    try {
      const response = await fetch(`/api/projects/${project.id}/lrc`);
      if (!response.ok) throw new Error('LRC 파일을 불러오지 못했습니다.');
      const blob = new Blob([await response.arrayBuffer()], { type: 'text/plain;charset=utf-8' });
      const suggestedName = `${project.title.replace(/[\\/:*?"<>|]+/g, ' ').trim() || '가사'}.lrc`;
      const picker = (window as unknown as { showSaveFilePicker?: SaveFilePickerFn }).showSaveFilePicker;
      if (typeof picker === 'function') {
        const handle = await picker({ suggestedName });
        const writable = await handle.createWritable();
  ...
  function seek(seconds: number) {
    ...
    if (!audio) return;
    audio.currentTime = Math.max(0, seconds - 0.2);
    ...
    <DialogContent className="studio-dialog lyric-sync-dialog">
      ...
      {loading ? <p className="field-hint"><LoaderCircle className="spin" size={14}/> 불러오는 중…</p> : <>
        ...
        <div className="lyric-sync-actions">
          ...
          {running && <span className="field-hint">처리 중… 창을 닫지 마세요.</span>}
        </div>
        {errorText && <p className="field-hint warning">{errorText}</p>}
        {sync && <>
...
// "LoRA 관리": installed LoRA adapters (with what they are, not only their file names), a curated catalog and a Hugging Face
// browser that classifies the YuE2 adapters found there. Adapters are applied by the second song engine (yue-server).
type AdapterItem = {
...
function AdapterCard({ item, onChanged, notify, running, setRunning }: { item: AdapterItem; onChanged: () => void; notify: (text: string, error?: boolean) => void; running: boolean; setRunning: (value: boolean) => void }) {
  const [editing, setEditing] = useState(false);
  const [title, setTitle] = useState(item.displayName);
  ...
  async function verify() {
    ...
    try {
      const result = await api<{ verified: { ok: boolean } }>(`/adapters/${encodeURIComponent(item.name)}/verify`, 'POST', {});
      notify(result.verified.ok ? '이 LoRA를 엔진이 읽을 수 있습니다.' : '엔진이 이 LoRA를 읽지 못했습니다. 다른 모델용 파일일 수 있습니다.', !result.verified.ok);
      onChanged();
    } catch (error) { notify((error as Error).message, true); }
  ...
  return <article className="adapter-card">
    ...
    <div className="adapter-meta">
      ...
      {item.trigger && <span className="adapter-chip trigger" title="스타일에 이 단어를 넣으면 반응합니다">트리거 {item.trigger}</span>}
      {item.categories.filter(label => label !== KIND_LABEL[item.kind]).map(label => <span key={label} className="adapter-chip">{label}</span>)}
      {item.languages.map(label => <span key={label} className="adapter-chip lang">{label}</span>)}
      {item.verified && <span className={`adapter-chip ${item.verified.ok ? 'ok' : 'bad'}`}>{item.verified.ok ? '엔진 확인됨' : '읽기 실패'}</span>}
      {item.commercialUse === false && <span className="adapter-chip warn">비상업용</span>}
    </div>
...
function InstallBar({ job, label }: { job: InstallJob | null; label: string }) {
  if (!job) return null;
  const percent = job.total ? Math.min(100, Math.round(job.downloaded / job.total * 100)) : 0;
  return <div className="adapter-install-bar" role="status"><LoaderCircle className="spin" size={14}/><span>{label} {formatSize(job.downloaded)} / {formatSize(job.total)} ({percent}%)</span><Progress value={percent}/></div>;
}
...
function AdapterPage({ notify, picker }: { notify: (text: string, error?: boolean) => void; picker?: AdapterPickMode }) {
  ...
  useEffect(() => { if (tab === 'hub' && !hub && !hubLoading) void loadHub(); }, [tab]); // eslint-disable-line react-hooks/exhaustive-deps

  // Starts a background download and follows it until it finishes.
  async function runInstall(route: string, payload: unknown) {
    if (job) return;
    setJob({ status: 'downloading', downloaded: 0, total: 0, names: [], error: '' });
    try {
      const { jobId } = await api<{ jobId: string }>(route, 'POST', payload);
      for (;;) {
  ...
  return <section className={`adapter-page${picker ? ' picking' : ' page-scroll'}`}>
    ...
    {tab === 'mine' && <>
      <div className="adapter-import"><strong><FolderOpen size={15}/> 파일 가져오기</strong>
        <div className="adapter-import-main">
          ...
          <div className="adapter-import-namerow">
            {importPicked.length > 0 ? <ul className="adapter-import-files">{importPicked.map(file => <li key={file.name}><span>{file.name}</span><small>{formatSize(file.size)}</small><button type="button" aria-label={`${file.name} 빼기`} onClick={() => setImportPicked(importPicked.filter(item => item !== file))} disabled={Boolean(importing)}><X size={12}/></button></li>)}</ul> : <div className="adapter-import-files empty">선택한 파일이 여기에 표시됩니다</div>}
    ...
    {picker && <div className="adapter-pick-footer"><span>{picked.length ? `${picked.length}개 선택됨` : '선택한 LoRA가 없습니다'}</span><Button variant="outline" onClick={picker.onClose}>취소</Button><Button onClick={() => picker.onApply(picked.filter(name => (mine?.adapters || []).some(item => item.name === name)))}><Check size={15}/>적용하기</Button></div>}
  </section>;
}
...
function AdapterPicker({ selected, onChange, style, onInsertTrigger, notify }: { selected: AdapterChoice[]; onChange: (next: AdapterChoice[]) => void; style: string; onInsertTrigger: (trigger: string) => void; notify: (text: string, error?: boolean) => void }) {
  ...
  const apply = (names: string[]) => {
    onChange(names.map(name => selected.find(entry => entry.name === name) || { name, arScale: DEFAULT_ADAPTER_SCALE, narScale: DEFAULT_ADAPTER_SCALE }));
    setOpen(false);
  };
  const chosen = selected.filter(entry => byName.has(entry.name));
  return <div className="wide-field adapter-picker"><div className="adapter-picker-head"><span className="field-label">LoRA{chosen.length > 0 && <small> {chosen.length}개 적용</small>}</span><Button type="button" variant="outline" size="sm" onClick={() => setOpen(true)}><Layers size={14}/>LoRA 선택</Button></div>
    {chosen.length === 0 ? <span className="vocal-gender-hint">적용한 LoRA가 없습니다. "LoRA 선택"에서 받은 LoRA를 고르거나 새로 받을 수 있습니다.</span> : chosen.map(entry => {
      ...
      return <div key={item.name} className="adapter-pick">
        <div className="adapter-pick-row"><div className="adapter-pick-name"><strong>{item.displayName}</strong><small>{item.kind ? `${KIND_LABEL[item.kind] || item.kind} · ` : ''}{STAGE_LABEL[item.stage] || item.stage}</small></div>
          <div className="adapter-pick-scales">
            {hasAr && <label className="adapter-scale"><em>작곡</em><ScaleInput value={entry.arScale} label={`${item.displayName} 작곡 강도`} onChange={value => setScale(item.name, 'arScale', value)}/></label>}
            {hasNar && <label className="adapter-scale"><em>사운드</em><ScaleInput value={entry.narScale} label={`${item.displayName} 사운드 강도`} onChange={value => setScale(item.name, 'narScale', value)}/></label>}
          </div>
          <button type="button" className="adapter-icon-btn" aria-label={`${item.displayName} 빼기`} title="빼기" onClick={() => onChange(selected.filter(other => other.name !== item.name))}><X size={15}/></button></div>
        {item.trigger && !style.includes(item.trigger) && <button type="button" className="link-button" onClick={() => onInsertTrigger(item.trigger)}>스타일에 "{item.trigger}" 넣기</button>}
    ...
    {open && <Dialog open onOpenChange={next => { if (!next) setOpen(false); }}>
      <DialogContent className="studio-dialog adapter-picker-dialog">
        ...
        <DialogDescription>곡에 쓸 LoRA를 체크하고 "적용하기"를 누르세요. 여러 개를 고를 수 있고, 새 LoRA는 Preset이나 허깅페이스 탭에서 받을 수 있습니다.</DialogDescription>
        <div className="adapter-picker-body"><AdapterPage notify={notify} picker={{ selected: selected.map(entry => entry.name), onApply: apply, onClose: () => setOpen(false) }}/></div>
      </DialogContent>
    </Dialog>}
  </div>;
}
    ...
    <DialogContent className="studio-dialog audio-compare-dialog timbre-transform-dialog ai-polish-dialog">
      ...
      <div className="timbre-transform-body">
        <div className="timbre-left-panel">
          ...
          <div className="polish-presets"><span>세기</span><Button size="sm" variant="outline" disabled={running} onClick={() => setSettings(previous => ({ ...POLISH_DEFAULT, ...{ master: previous.master }, denoise: { ...POLISH_DEFAULT.denoise, enabled: true }, lifter: { ...POLISH_DEFAULT.lifter, enabled: true }, naturalize: { ...POLISH_DEFAULT.naturalize, enabled: true } }))}>기본값으로 세 단계</Button><Button size="sm" variant="outline" disabled={running} onClick={() => setSettings(previous => ({ ...POLISH_STRONG, master: previous.master }))} title="효과가 실제로 있는지 귀로 확인하기 위한 강한 설정">강하게 (효과 확인용)</Button></div>
          <div className="polish-step">
            <label className="at-function"><input type="checkbox" checked={settings.denoise.enabled} onChange={event => patch('denoise', { enabled: event.target.checked })} disabled={running}/>노이즈 제거</label>
...
export default function Studio() {
  ...
  // bar colour by usage: green, yellow above 70 %, red above 90 %
  const usageLevel = (percent: number) => (percent > 90 ? 'high' : percent > 70 ? 'mid' : '');
  // resource usage in the top bar (GPU / VRAM / CPU / RAM), polled every 2 s while this tab is visible
  const [stats, setStats] = useState<{ gpu: { name: string; utilization: number | null; vramUsedMb: number | null; vramTotalMb: number } | null; cpuPercent: number | null; ramUsedMb: number; ramTotalMb: number } | null>(null);
  useEffect(() => {
    let stopped = false;
    const poll = () => { if (document.visibilityState === 'visible') api<NonNullable<typeof stats>>('/system/stats').then(next => { if (!stopped) setStats(next); }).catch(() => { if (!stopped) setStats(null); }); };
    poll();
  ...
  const [lyricSyncTarget, setLyricSyncTarget] = useState<Project | null>(null);
  ...
  // height of the player-bar spectrum bars (the ^ / v buttons on its right); remembered per browser
  const [waveGain, setWaveGain] = useState(() => { try { const saved = Number(localStorage.getItem('songyue2-wave-gain')); return saved >= 0.3 && saved <= 4 ? saved : 1; } catch { return 1; } });
  const waveGainRef = useRef(waveGain);
  useEffect(() => { waveGainRef.current = waveGain; try { localStorage.setItem('songyue2-wave-gain', String(waveGain)); } catch { /* per-browser convenience only */ } }, [waveGain]);
  ...
  async function chooseInstrumental(on: boolean) {
  ...
  // --- playback queue ---
  // startIndex points into `songs`, which may also hold drafts; the queue only holds completed songs, so the start position
  // is looked up by the clicked song itself (indexing the filtered list with the unfiltered index plays a different song).
  function playQueue(songs: Project[], startIndex = 0) {
    ...
    const found = clicked ? playable.findIndex(song => song.id === clicked.id) : -1;
    const clampedIndex = found >= 0 ? found : Math.max(0, Math.min(startIndex, playable.length - 1));
    ensureMainAnalyser();
    ...
    const tick = () => {
      ...
      if (canvas && ctx2d) {
        ...
        if (playing && width > 0 && height > 0) {
          // Frequency bars like the microphone monitor of the timbre dialog: X = 0-10 kHz, Y = magnitude per band.
          const ratio = 1;
          const axisHeight = 0;
          const plotHeight = height - axisHeight;
          const frame = new Uint8Array(analyser?.frequencyBinCount || 1);
          if (analyser) analyser.getByteFrequencyData(frame);
          const binHz = (mainAudioCtxRef.current?.sampleRate || 48000) / (analyser?.fftSize || 2048);
          const maxHz = 10000;
          const maxBin = Math.max(2, Math.min(frame.length - 1, Math.floor(maxHz / binHz)));
          const barWidth = Math.max(3, width / 80);
          ctx2d.fillStyle = `hsla(${settings.visualizerHue % 360}, 85%, 70%, 0.9)`;
          for (let x = 0; x < width; x += barWidth) {
            const bin = Math.max(1, Math.round((x / width) * maxBin));
            const magnitude = Math.min(1, 0.8 * Math.pow(frame[bin] / 255, 1.6) * waveGainRef.current);
            ctx2d.fillRect(x, plotHeight - magnitude * plotHeight, Math.max(1, barWidth - 1.5), magnitude * plotHeight);
          }
        } else if (!playing) {
  ...
  // Synced lyrics of the playing song (made by "가사 싱크"); the current line is shown in the player bar.
  const [nowLyrics, setNowLyrics] = useState<LyricSync | null>(null);
  useEffect(() => {
    setNowLyrics(null);
    if (!nowPlaying?.id) return;
    let cancelled = false;
    api<LyricSync>(`/projects/${nowPlaying.id}/lyrics-sync`).then(result => { if (!cancelled && !result.stale) setNowLyrics(result); }).catch(() => {});
    return () => { cancelled = true; };
  }, [nowPlaying?.id]);
  const nowLyricIndex = nowLyrics ? currentLyricIndex(nowLyrics.lines, playbackTime) : -1;
  ...
  async function handleCoverFile(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      if (target.type === 'project') {
        const { coverNotice: lowNotice, ...result } = await api<Project & { coverNotice?: string }>(`/projects/${target.item.id}/cover`, 'POST', { dataUrl });
        setProjects(previous => previous.map(project => project.id === target.item.id ? result : project));
        if (lowNotice) { notify(`커버 이미지를 등록했습니다. ${lowNotice}`); return; }
      } else {
        const { coverNotice: lowNotice, ...result } = await api<AbcNote & { coverNotice?: string }>(`/abc-notes/${target.item.id}/cover`, 'POST', { dataUrl });
  ...
  return <div className="studio-shell">
    ...
    <main className="main-shell"><header className="topbar"><div className="topbar-title"><Button variant="ghost" size="icon" className="mobile-menu" aria-label="메뉴 열기" onClick={() => setMobileNav(true)}><Menu/></Button><span className="breadcrumb">작업 공간</span><ChevronRight size={14}/><strong>{titles[page]}</strong></div><Popover open={modelOpen} onOpenChange={setModelOpen}><PopoverTrigger render={<Button variant="outline" className="model-trigger" aria-label="음악 모델 선택"/>}><AudioLines size={17}/><span>{model.name}</span><span className="model-recommended">{model.id === 'yue2-q8' ? '추천' : model.engine}</span><ChevronDown size={15}/></PopoverTrigger><PopoverContent className="model-menu" align="start"><div className="menu-heading">음악 생성 모델<span>새 작업에 적용할 모델을 선택하세요</span></div>{models.map(item => { const missing = !!inventory && !installed(item); return <button key={item.id} className={`model-option ${draft.modelId === item.id ? 'selected' : ''}`} style={missing ? { opacity: 0.6 } : undefined} disabled={item.selectable === false} onClick={() => { if (item.selectable === false) return; if (missing) { notify(`${item.name} 모델은 다운로드가 필요합니다. 모델 파일을 내려받은 뒤 선택해 주세요.`, true); return; } update({ modelId: item.id }); setModelOpen(false); }}><Cpu size={18}/><span><strong>{item.name}<em>{missing ? '다운로드 필요' : item.badge}</em></strong><small>{item.detail} · {item.size}</small></span>{draft.modelId === item.id && <Check size={17}/>}</button>; })}<div className="model-menu-footer">모델 파일과 실행 엔진의 준비 상태는 별도로 확인합니다.<button onClick={() => { navigate('models'); setModelOpen(false); }}>모델 관리 <ArrowRight size={13}/></button></div></PopoverContent></Popover><div className="device-status"><span className={`status-dot ${online ? '' : 'offline'}`}/><span>{online ? '로컬 연결됨' : '로컬 연결 대기'}</span><span className="device-divider"/><Cpu size={14}/><span>{stats?.gpu?.name || 'RTX 5070'} <span className="muted">· {stats?.gpu ? Math.round(stats.gpu.vramTotalMb / 1024) : 12} GB</span></span>{stats && <span className="resource-usage" aria-label="자원 사용 현황">
      ...
      {stats.gpu && stats.gpu.vramUsedMb !== null && <span title={`GPU 메모리(VRAM) 사용량 ${(stats.gpu.vramUsedMb / 1024).toFixed(1)} / ${Math.round(stats.gpu.vramTotalMb / 1024)} GB`}><b><em>VRAM</em>{Math.round(stats.gpu.vramUsedMb / stats.gpu.vramTotalMb * 100)}%</b><i style={{ width: `${Math.min(100, stats.gpu.vramUsedMb / stats.gpu.vramTotalMb * 100)}%` }} className={usageLevel(stats.gpu.vramUsedMb / stats.gpu.vramTotalMb * 100)}/></span>}
      {stats.cpuPercent !== null && <span title="CPU 사용률"><b><em>CPU</em>{stats.cpuPercent}%</b><i style={{ width: `${Math.min(100, stats.cpuPercent)}%` }} className={usageLevel(stats.cpuPercent)}/></span>}
      <span title={`메인 메모리(RAM) 사용량 ${(stats.ramUsedMb / 1024).toFixed(1)} / ${Math.round(stats.ramTotalMb / 1024)} GB`}><b><em>RAM</em>{Math.round(stats.ramUsedMb / stats.ramTotalMb * 100)}%</b><i style={{ width: `${Math.min(100, stats.ramUsedMb / stats.ramTotalMb * 100)}%` }} className={usageLevel(stats.ramUsedMb / stats.ramTotalMb * 100)}/></span>
    </span>}</div></header>
    {page === 'create' ? <div className="creation-layout"><section className="composer" aria-label="노래 편집기"><div className="composer-scroll"><div className="composer-heading"><div><h1>어떤 노래를 만들까요?</h1></div><Music2 size={24}/></div><div className="mode-switch" aria-label="제작 모드"><button className={draft.mode === 'simple' ? 'active' : ''} aria-pressed={draft.mode === 'simple'} onClick={() => update({ mode: 'simple' })}>간편 모드</button><button className={draft.mode === 'custom' ? 'active' : ''} aria-pressed={draft.mode === 'custom'} onClick={() => update({ mode: 'custom' })}>직접 만들기<SlidersHorizontal size={14}/></button></div><div className="mode-switch vocal-mode-switch" aria-label="보컬 여부"><button className={!draft.instrumental ? 'active' : ''} aria-pressed={!draft.instrumental} onClick={() => void chooseInstrumental(false)}><Mic size={14}/>보컬+악기</button><button className={draft.instrumental ? 'active' : ''} aria-pressed={draft.instrumental} onClick={() => void chooseInstrumental(true)}><Guitar size={14}/>악기만</button></div>
    ...
    {lyricSyncTarget && <LyricsSyncDialog project={lyricSyncTarget} onClose={() => setLyricSyncTarget(null)} notify={notify}/>}
