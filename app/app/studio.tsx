...
import { api, emptyDraft, initialSettings, models, providers, saveFormats, viewModes, titles, gb, DEFAULT_SETTING_PATH, DEFAULT_MUSIC_PATH, DEFAULT_EXAMPLES_PATH, DEFAULT_COVERS_PATH, DEFAULT_ABC_NOTES_PATH, DEFAULT_STYLE_PRESETS, DEFAULT_VISUALIZER_RING_COUNT, DEFAULT_VISUALIZER_HUE, DEFAULT_VISUALIZER_LINE_WIDTH, DEFAULT_VISUALIZER_TRAIL, DEFAULT_VISUALIZER_SPIRAL, DEFAULT_VISUALIZER_RING_MODE, DEFAULT_ENGINE_PATH, PYTHON_MODEL_MIN_VRAM_MB, type Page, type Draft, type Project, type Settings, type Inventory, type Example, type Playlist, type AbcNote, type SaveFormat, type SystemInfo } from './studio-data';
...
function PostProcessDialog({ project, onClose, notify, visualizerEnabled, visualizerRingCount, visualizerHue, visualizerLineWidth, visualizerTrail, visualizerSpiral, visualizerRingMode }: { project: Project; onClose: () => void; notify: (text: string, error?: boolean) => void; visualizerEnabled: boolean; visualizerRingCount: number; visualizerHue: number; visualizerLineWidth: number; visualizerTrail: number; visualizerSpiral: number; visualizerRingMode: 'radial' | 'time' }) {
  ...
  useEffect(() => {
    ...
    // Ring i's shape comes from the i-th most recent analyser frame (0 = now, 1 = one frame ago, ...)
    // rather than a rotated slice of the same frame, so ring index is a real time axis.
      ...
      if (canvas && ctx2d) {
        ...
        if (playing) {
          ...
          if (visualizerRingMode === 'time') {
            ...
            if (history.length > ringCount) history.length = ringCount;
          }
          for (let ringIndex = 0; ringIndex < rings.length; ringIndex++) {
            const ring = rings[ringIndex];
            // Mode 1 (radial): every ring reads the same current frame, offset by bin position.
            // Mode 2 (time): ring i reads the i-th most recent frame -- ring index is a time axis.
            const source = visualizerRingMode === 'time' ? history[ringIndex] : frame;
            if (!source) continue;
  ...
  }, [visualizerEnabled, visualizerRingCount, visualizerHue, visualizerLineWidth, visualizerTrail, visualizerSpiral, visualizerRingMode]);
...
export default function Studio() {
  ...
  return <div className="studio-shell">
    ...
    {postProcessTarget && <PostProcessDialog project={postProcessTarget} onClose={() => setPostProcessTarget(null)} notify={notify} visualizerEnabled={settings.visualizerEnabled} visualizerRingCount={settings.visualizerRingCount} visualizerHue={settings.visualizerHue} visualizerLineWidth={settings.visualizerLineWidth} visualizerTrail={settings.visualizerTrail} visualizerSpiral={settings.visualizerSpiral} visualizerRingMode={settings.visualizerRingMode}/>}
  </div>;
}
