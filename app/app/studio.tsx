...
function AdapterPage({ notify }: { notify: (text: string, error?: boolean) => void }) {
...
function AdapterPicker({ selected, onChange, openManager, style, onInsertTrigger }: { selected: AdapterChoice[]; onChange: (next: AdapterChoice[]) => void; openManager: () => void; style: string; onInsertTrigger: (trigger: string) => void }) {
  const [list, setList] = useState<AdapterList | null>(null);
  useEffect(() => { api<AdapterList>('/adapters').then(setList).catch(() => setList(null)); }, []);
  if (!list) return null;
  ...
  return <div className="wide-field adapter-picker"><span className="field-label">LoRA <button type="button" className="link-button" onClick={openManager}>관리·받기</button></span>
    {list.adapters.length === 0 ? <span className="vocal-gender-hint">받은 LoRA가 없습니다. "관리·받기"에서 Preset이나 허깅페이스의 LoRA를 받을 수 있습니다.</span> : list.adapters.map(item => {
      ...
      return <div key={item.name} className="adapter-pick"><label><input type="checkbox" checked={Boolean(chosen)} onChange={event => toggle(item, event.target.checked)}/><span>{item.displayName}</span><small>{item.kind ? `${KIND_LABEL[item.kind] || item.kind} · ` : ''}{STAGE_LABEL[item.stage] || item.stage}</small></label>
        {chosen && <div className="adapter-scales">
          {hasAr && <span className="adapter-scale"><em>작곡</em><input type="range" min={0} max={1.5} step={0.05} value={chosen.arScale} aria-label={`${item.displayName} 작곡 강도`} onChange={event => setScale(item.name, 'arScale', Number(event.target.value))}/><b>{chosen.arScale.toFixed(2)}</b></span>}
          {hasNar && <span className="adapter-scale"><em>사운드</em><input type="range" min={0} max={1.5} step={0.05} value={chosen.narScale} aria-label={`${item.displayName} 사운드 강도`} onChange={event => setScale(item.name, 'narScale', Number(event.target.value))}/><b>{chosen.narScale.toFixed(2)}</b></span>}
    ...
    {selected.length > 0 && <span className="vocal-gender-hint">LoRA를 쓰면 곡이 별도 엔진(yue-server)으로 만들어지며, "악기만" 모드는 지원하지 않습니다. 추론 단계는 32 정도를 권장합니다.</span>}
  </div>;
}
