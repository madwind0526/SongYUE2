...
export default function Studio() {
  ...
  const update = (value: Partial<Draft>) => setDraft(previous => {
    const next = { ...previous, ...value };
    // GGUF models only apply "악기만" as a soft style hint (no structural guarantee), so
    // switching away from the 원본 (Python) model drops any instrumental selection instead
    // of leaving it silently stuck on a model that can't really honor it.
    if (value.modelId !== undefined && models.find(item => item.id === value.modelId)?.engine !== 'Python') next.instrumental = false;
    return next;
  });
  ...
  useEffect(() => {
    try {
      ...
      if (cached && typeof cached.lyrics === 'string') {
        const merged = { ...emptyDraft, ...cached };
        if (models.find(item => item.id === merged.modelId)?.engine !== 'Python') merged.instrumental = false;
        setDraft(merged);
      }
  ...
  function loadProject(item: Project) { const loadedModel = models.find(entry => entry.id === item.modelId); setDraft({ title: item.title, lyrics: item.lyrics, style: item.style, modelId: item.modelId, seed: item.seed, steps: item.steps, cot: item.cot, vocalGender: item.vocalGender || '', instrumental: Boolean(item.instrumental) && loadedModel?.engine === 'Python', abc: item.abc || '', mode: item.mode }); setSelected(null); navigate('create'); notify('설정을 불러왔습니다. 다시 저장하면 새 버전으로 남습니다.'); }
  ...
  function coverFromProject(item: Project) {
    // What matters is the model that will actually consume the ABC (the composer's current
    // selection), not whatever model rendered this song's audio -- SheetSage2 transcribes any
    // audio file regardless of which engine produced it.
    if (model.engine !== 'Python') return;
  ...
  async function runCover(item: Project, useItemSettings: boolean) {
    ...
    try {
      ...
      const result = await api<{ abc: string }>('/cover-transcribe', 'POST', { dataUrl, task: 'melody-full' });
      // modelId is deliberately left untouched: it's already validated Python (the "커버" button
      // is disabled otherwise), and copying the source song's own model could silently swap it
      // back to a GGUF model that can't use the abc we're about to set.
