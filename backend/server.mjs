  ...
  const server = http.createServer(async (req, res) => {
    ...
    try {
      ...
      if (pathname.startsWith('/api/lora-train')) {
        ...
        if (req.method === 'GET' && sub === '/browse') return run(() => browseFolders(String(requestUrl.searchParams.get('path') || '').trim() === '@library' ? loraTrainer.libraryDir() : String(requestUrl.searchParams.get('path') || '').trim()).catch((error) => { throw Object.assign(error, { status: 400 }); }));
