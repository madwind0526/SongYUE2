...
test('AI 곡 다듬기: 완성곡을 처리기에 넘겨 미리듣기를 만들고, 저장하면 새 곡으로 라이브러리에 추가하며, 버리면 지운다', async t => {
  ...
  // Audio Tools "AI 처리": the same chain on a data URL; no reference mastering, at least one stage, a wav data URL comes back
  const toolAudio = `data:audio/wav;base64,${Buffer.from('fake-speech').toString('base64')}`;
  assert.equal((await callJson('/api/audio-tools/polish', 'POST', { settings: { denoise: { enabled: true } } })).status, 400);
  const noStage = await callJson('/api/audio-tools/polish', 'POST', { audioDataUrl: toolAudio, settings: {} });
...
test('FX 프리셋(FX Sound + 리버브/에코): 저장, 목록, 삭제와 값 검사', async t => {
  resetEnv();
  ...
  assert.equal((await call('/api/effect-presets', 'POST', { name: '나쁜 값', params: { ...params, bassBoost: 'x' } })).status, 400);
  ...
  assert.equal('compRatio' in list[0].params, false, 'only FX / reverb values are stored');
  assert.equal('eq' in list[0].params, false, 'the EQ has its own presets');
