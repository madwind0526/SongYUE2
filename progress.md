...
## 2026-09-26 추가: 아직 검증/정리가 필요한 것

- **VST3**: Dragonfly Reverb 외 플러그인(TDR Nova/Kotelnikov, Valhalla Supermassive, Kilohearts Essentials)은 이 호스트에서 창이 열리고 처리되는지 **시험하지 않았다**. Audio Tools의 "AI 처리" 창에는 VST3 UI가 없다(서버는 받을 수 있음). 설정 창의 "강제 종료"는 상태를 저장하지 못할 수 있다. `engine/`이 git 밖이라 새 PC에서는 `vst-host.exe`를 직접 넣어야 한다.
- **곡 표지 자동 생성**: Z-Image 그림은 스타일에만 맞고 가사와는 무관하다(한글이 글자로 그려지는 문제 때문에 제목·가사를 프롬프트에서 뺌). 가사를 영어 장면 설명으로 바꾸려면 LLM이 필요해 보류. Pixabay 사진은 곡과 어긋나는 경우가 있다(예: "night stars" → 크리스마스트리). `engine/ComfyUI/extra_model_paths.yaml`은 이 PC의 D:\sd-lib를 가리키며 git에 없다.
- **LoRA 학습**: 캡션(.txt) 작업 폴더 처리는 기본 동작만 확인. 보관함의 `김현식 rank16/rank32`, 비교용 곡들(김현식 비교/여성보컬 ...)은 정리 대상(rank 16만 남기는 것을 권장). LoRA 강도(0.7/1.0/1.5) 비교는 하지 않았다.
- **문서/버전**: 앱 버전 1.0. 이 절 아래의 "다음 에이전트용 상세 계획"은 2026-09-24 기준이라 AuK 제거와 Audio Tools 재구성 위주다.

## 보컬 자연화 실측 (2026-09-26, 라이브러리 곡 16개 × 20초, 기본 세기 0.5)

`backend/postfx/naturalize.mjs`를 곡마다 단계 하나씩만 켜서 측정했다(스크립트는 `scratch/nat/`, git 밖). 결론: **자연화는 사실상 "고음을 살짝 줄이는 처리"**이고 보컬 전용 효과가 아니다(연주곡에서도 같음).

- 기본 전체: 4~6kHz −0.5~−1.2 dB, 6~10kHz −1.8~−2.4 dB, 10~16kHz −1.9~−2.5 dB, 16~22kHz −2.9~−3.7 dB(곡 간 편차 작음). 세기 1.0이면 6~10kHz −3.4~−4.2 dB, 16kHz 이상 −4~−7 dB.
- 소리 나는 단계는 금속성 감소(6~10kHz −1.0~−1.6 dB)와 전환 부드럽게(6kHz 이상 −1~−3.4 dB의 고음 롤오프) 둘뿐이다.
- **미세 흔들림("비브라토")과 보컬 질감("포먼트")은 들리지 않는다**: 곡 대비 차이 −103 dB / −77~−79 dB(상수가 너무 작음). 피크가 1.0인 곡에서 −26 dB로 나오는 것은 이 단계가 아니라 피크 제한(0.95) 게인 때문이다.
- 피크가 0.95를 넘는 곡(16개 중 8개, 최대 1.0)은 전체 음량이 −0.4~−0.5 dB 줄어든다(나머지는 0.00 dB). 원본과 비교할 때 음량 맞추기가 필요하다.
- 결정 대기: ① 화면 설명을 실제 동작(고음 금속성 줄이기, 부드럽게)에 맞게 고치기 ② 소리 나는 옵션 둘만 "고급 설정"으로 노출 + 은은하게/기본/강하게 ③ 진짜 음높이 흔들림·포먼트 이동 구현(실험).
