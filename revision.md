...
앱 버전 표기를 0.1에서 **1.0**으로 올렸다(사이드바, `package.json`, `app/package.json`, `app/package-lock.json`, `/api/health`의 `version`). 이 아래 항목들은 09-20 이후 커밋을 주제별로 묶은 것이다(커밋 약 190개).

## VST3 플러그인: AI 곡 다듬기 단계 + "VST3 관리" 페이지 (2026-09-26, `79603c2` `e542251` `c809ce5` `f265bb0` 외)

HOT-Step-CPP의 `vst-host.exe`(MIT)를 `engine/vst-host/`에 두고 VST3 플러그인을 별도 프로세스로 돌린다. **AI 곡 다듬기**의 처리 순서는 노이즈 제거 → Spectral Lifter → 보컬 자연화 → **VST3** → 기준곡 마스터링이 되었다. 곡 다듬기 창에서 플러그인을 고르면 바로 체인에 추가되고, 줄마다 켜기/끄기, 플러그인 자체 설정 창(닫을 때 `Setting/VST-states/`에 상태 저장), 위/아래, 제거, 체인 프리셋 불러오기/저장이 있다. 왼쪽 열이 넘치면 세로 스크롤만 된다. 처리는 워커 스레드에서 float32 WAV로 호스트에 넘기고(길이는 항상 입력과 같게), 5분 제한과 프로세스 강제 종료, 검색된 플러그인만 허용하는 검사가 있다.

왼쪽 메뉴에 **VST3 관리**(음색 변조 아래)가 생겼다: 플러그인 카드(설정, 시험 듣기, 저장된 설정 지우기, 앱 폴더 플러그인 삭제), 체인 프리셋 관리, 추천 무료 플러그인 링크, 폴더 열기. **시험 듣기**는 곡의 일부(5~60초)에 플러그인 하나를 저장된 설정으로 적용해 원본 구간과 파형 플레이어로 번갈아 듣게 하고, "음량 맞추기"(기본 켜짐)로 처리한 소리를 원본 음량에 맞춰 재생한다. 플러그인은 표준 VST3 폴더와 `engine/vst-host/plugins/`에서 찾는다(앱 폴더는 앱이 직접 검색). Dragonfly Reverb(Hall/Plate)로 검색·설정 창·상태 저장·곡 처리·시험 듣기를 실제로 확인했고, 저장된 설정이 처리에 쓰이는지 측정했다(설정 유무 −14.6 dB 차이, 화면 결과와 호스트 직접 실행 −121.8 dB). 다른 플러그인은 무료·VST3·Windows 여부만 확인했고 이 호스트에서의 동작은 시험하지 않았다. 백엔드 테스트 `backend/vst.test.mjs`(가짜 호스트) 7개 추가, 전체 86개. 자세한 내용은 `docs/vst3.md`.

왼쪽 메뉴에서 **내 홈**을 없애고(`b50cb04`, 내 라이브러리와 같은 목록이었음) **Audio Tools**를 LoRA 관리 아래로 옮겼다.

## 곡 표지 자동 생성 (2026-09-26, `1ef52e0` `f8b1c98` `bbc06bd`)

새 곡이 저장될 때 표지가 없으면 자동으로 만든다: ① ComfyUI의 **Z-Image Turbo**(1536px, nvfp4 → fp8 → bf16 순으로 있는 파일 사용) ② **Pixabay 무료 사진**(`.env`의 `PIXABAY_API_KEY`) ③ 제목과 장르가 들어간 **그래픽**(ffmpeg). 프롬프트에는 음악 스타일의 영어 단어와 가수 성별만 쓴다 — 제목이나 가사를 넣으면 한글이 그림 속 글자로 그려지고, cfg 2.0의 negative prompt로도 막지 못했다(직접 시험). Pixabay는 한글 검색이 안 되어(10개 단어 전부 0건) 가사의 한글 단어를 영어 검색어로 바꾸는 사전(약 70개)을 만들고, 정사각형에 가까운 사진을 고른 뒤 자르고 1400px로 키운다(Pixabay는 긴 변 1280px 파일만 준다). 직접 올린 표지도 가운데 기준 정사각형으로 자르고 2475px를 넘으면 줄이며 1400px보다 작으면 안내한다. 설정에 "곡을 만들 때 표지를 자동으로 만들기"(기본 켜짐)가 있다. `backend/cover-art.mjs`, 테스트 `backend/cover-art.test.mjs`.

## LoRA 학습 탭 (2026-09-26, `70110e1` `390217c` `23d4096` `972273c` `1c87759` `cfaaab5`)

"LoRA 관리 → 학습" 탭에서 내 곡으로 사운드(NAR) LoRA를 학습한다(ComfyUI-YuE2-Trainer 노드, BF16 체크포인트 필요). 곡 폴더 찾기(한 번 클릭 선택, 더블 클릭 진입, `library/Lora`에서 시작), 곡 목록에서 일부만 선택, 조각 길이 직접 입력(2~10초, 기본 6초 — 12GB GPU 한계), rank 16/32(기본 16), 끝내는 조건(스텝 또는 곡 전체 반복 횟수), 진행률·남은 시간·취소, 학습 후 EMA/raw 차이를 자동으로 재서 다를 때만 A/B 시험곡, 확정 시 `library/Lora/<이름>/`에 보관하고 Installed에 하드 링크로 등록한다. 보관함에서 이름·메모 수정과 삭제가 된다. 실측: 지수 4곡 1500스텝(곡 전체 약 12.5회), 김현식 17곡 1500스텝(약 2.2회) 모두 효과가 확인되었고 rank 16과 32는 측정값 차이가 없어 rank 16을 기본으로 했다. 학습기 GitHub 링크와 비상업 라이선스(CC BY-NC 4.0) 안내를 LoRA 관리 설명에 넣었다. 자세한 내용은 `docs/lora-training.md`.

## LoRA 관리 화면 정리 (2026-09-25, `1f79d58` `64e7333` `0ab9499` `56336e9` 외)

곡 만들기에서 LoRA 선택 창(적용 버튼, LoRA별 강도 0~2 기본 0.7), 악기만 선택 시 instrumental LoRA 자동 적용(GGUF/INT8 포함), 앱 모델이 Q8/INT8이면 Q8, BF16/원본이면 BF16 yue-server 사용. Hugging Face 탭(분류·언어 칩, 정렬 아이콘, 새로고침과 NEW 표시, 찾기 버튼), Preset/Installed 탭의 정렬·좋아요·분류·언어 필터, 카드는 검은 바탕에 회색 테두리, 업로드로 가져오기.

## 후처리, Audio Tools, 화면 (2026-09-25)

- **후처리 패널**: 컴프레서(5개 값, 프리셋 포함), 음량/무음제거/페이드/재생 속도/리버스 패널, 프리셋 분리(EQ, FX·리버브, 컴프레서, 전체)와 가나다·A-Z 정렬, 중앙 정렬 레이아웃, 모든 스위치를 켠 중립 시작값.
- **AI 처리 / AI 곡 다듬기**: Audio Tools, 음원 복원, 음색 변조, 음원 비교 결과 줄에 "AI 처리"와 후처리 버튼(원본과 처리 후를 파형으로 비교). 보컬 자연화 기본 켜짐.
- **TTS/STT**: VoxCPM2 제거(음성 성별 오류), 모델 순서 Qwen3/Fish/Chatterbox/OmniVoice/Typecast, "전처리" 체크박스(숫자·약어를 한글로 풀어 읽기), 미설치 모델 버튼은 검은 바탕에 회색 글씨, STT는 Qwen3-ASR 1.7B(INT8 기본)만.
- **화면**: 상단 바에 GPU/VRAM/CPU/RAM 사용률(초록, 70% 넘으면 노랑, 90% 넘으면 빨강, 좁으면 % 숫자만), 플레이어 바의 스펙트럼 높이 조절(^ / v), 내 라이브러리 기본 탭은 완성곡.
